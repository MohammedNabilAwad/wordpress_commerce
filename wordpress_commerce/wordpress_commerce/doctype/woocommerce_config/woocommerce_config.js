...
frappe.provide("wordpress_commerce.woocommerce_config");

frappe.ui.form.on("WooCommerce Config", {
    onload: function(frm, dt, dn){
        frappe.call({
            method:"wordpress_commerce.wordpress_commerce.doctype.woocommerce_config.woocommerce_config.get_series",
            callback:function(r){
    ...
    refresh: function(frm){
        ...
        if(!frm.doc.__islocal && frm.doc.enable_woocommerce === 1){
            ...
            frm.add_custom_button(__('Sync WooCommerce'), function() {
                frappe.call({
                    method:"wordpress_commerce.api.sync_woocommerce",
                })
            }).addClass("btn-primary");
            
            frm.add_custom_button(__("Sync WooCommerce IDs to ERP"), function(){
                frappe.call({
                    method:"wordpress_commerce.api.sync_woocommerce_ids",
                })
            })
        }
        ...
        frappe.call({
            method: "wordpress_commerce.api.get_log_status",
            callback: function(r) {
                if(r.message){
