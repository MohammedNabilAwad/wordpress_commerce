...
def sync_woocommerce():
    ...
    if timeout < 60:
        timeout = 60
    enqueue("wordpress_commerce.api.sync_woocommerce_resources", queue='long', timeout=timeout)
...
def sync_woocommerce_resources():
    ...
    elif frappe.local.form_dict.cmd == "wordpress_commerce.api.sync_woocommerce":
...
def sync_woocommerce_ids():
    "Enqueue longjob for syncing woocommerce"
    enqueue("wordpress_commerce.sync_products.add_w_id_to_erp", queue='long', timeout=1500)
    frappe.msgprint(_("Queued for syncing. It may take a few minutes to an hour if this is your first sync."))
